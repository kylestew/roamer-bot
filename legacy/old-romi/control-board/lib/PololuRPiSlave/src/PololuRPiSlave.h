...
private:
    ...
    void piDelay() { delayMicroseconds(pi_delay_us); }

    void updateI2CBuffer() { memcpy(&i2c_read_buffer, &staging_buffer, sizeof(BufferType)); }
...
public:
    ...
    virtual void receive(uint8_t b) {
        ...
        if (!index_set) {
            ...
            if (i2c_write_length > sizeof(i2c_write_buffer)) i2c_write_length = 0;
    ...
    virtual void stop() { finalizeI2CWrites(); }

    /* Initialize the slave on a given address. */
    void init(uint8_t address) { PololuTWISlave::init(address, *this); }
};
