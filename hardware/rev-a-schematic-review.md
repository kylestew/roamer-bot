...
- **Dedicated VDDA decoupling is omitted.** VDDA/VSSA remain connected to the generally decoupled +3V3/GND rails. ST identifies VDDA as the supply for the ADC, reset blocks, internal RC circuits, and PLL, and recommends a local 1 uF plus 10 nF pair. Rev A accepts the reduced local noise margin because BATLEV only needs a coarse reading. If clock, reset, or ADC instability appears during motor operation, add the capacitors in rework or restore them in the next revision.
...
- J3/J4 match the mirrored Romi 32U4 connector silkscreen. Viewed from the top, left J4 is `GND, ENC_L_A, ENC_L_B, VBAT_SW, ML+, ML-` from top to bottom; right J3 is `MR+, MR-, VBAT_SW, ENC_R_A, ENC_R_B, GND`. `VBAT_SW` intentionally replaces the Pololu board's `5V` encoder supply. Wheel-forward motor polarity and encoder count sign remain bring-up checks.
